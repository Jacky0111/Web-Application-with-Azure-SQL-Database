...
global $wpdb;
$results = $wpdb->get_results( "SELECT A.transID, C.productName, B.productQty, FORMAT(B.transactionSubtotalAmount, 2) AS transactionSubtotalAmount FROM transactions A, transactiondetails B, product C, customer D WHERE A.transID = B.transID AND B.productID = C.productID AND
                                     A.custID = D.custID and D.custID = $customerID AND transactionStatus = 'To Received' "); 
if(!empty($results)){                      // Checking if $results have some values or not
    ...
    foreach($results as $row){  
        ...
        elseif($new_transID == $old_transID){
            ...
            <tr>
               ...
               <td style ='border: none;float:right'>
                   <button type='submit' name='OrderReceived' value='$new_transID' style='color:white;font-size:20px;padding: 20px;background-color:#5FBED7;border: none;border-radius: 8px'>Order Received</button><br></br>
                </td>
            </tr>
            </form>";
        ...
        if (isset($_POST['OrderReceived']) && !empty($_POST['OrderReceived'])) {
            $set1 = array(transactionStatus => "Completed");
            $where1 = array("transID" => $_POST['OrderReceived']);
            $edit = $wpdb->update("transactions", $set1, $where1);

            if($edit){
                echo "<script>setTimeout(alert(\"Order Received Successfully!\"), 3000)</script>";
		        echo "<script> location.href='https://limchiachung.azurewebsites.net/completed/'; </script>";
            }
            else{
