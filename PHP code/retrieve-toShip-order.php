...
$results = $wpdb->get_results( "SELECT A.transID, C.productName, B.productQty, FORMAT(B.transactionSubtotalAmount, 2) AS transactionSubtotalAmount FROM transactions A, transactiondetails B, product C, customer D WHERE A.transID = B.transID AND B.productID = C.productID AND
                                     A.custID = D.custID and D.custID = $customerID AND transactionStatus = 'To Ship' "); 
if(!empty($results)){                      // Checking if $results have some values or not
    ...
    foreach($results as $row){  
        ...
        elseif($new_transID == $old_transID){
	        ...
	        echo "<tr><td colspan='3' style ='border: none;'> </td><td style ='border: none;float:right;'><center><a href='https://limchiachung.azurewebsites.net/order-cancellation-request/?transID=$new_transID'><input type='button' name='Cancel Order' value='Cancel Order' style='color:white;font-size:20px;padding: 20px;background-color:#5FBED7;border: none;border-radius: 8px'/></a></center> <br></br></td>";
            echo "<tr>
        	<th style=\"padding-left:10px;text-align:center;\"><font size = 4pt>Transaction ID</th>
        	<th style = 'text-align:center'><font size = 4pt>Product Name</th>
       		<th style = 'text-align:center'><font size = 4pt>Quantity</th>
